...
//회원가입 배너 슬라이더
   ...
   //팝업닫기
    ...
    // 담당자 클릭시 팝업닫기 2023.04.28 담당자 : 한상윤 [개발쪽 요청]
    ...
    //팝업에 버튼클릭시
    $('.popWrap table button').click(function(){
        const popBtn = $('.popWrap table button');
        popBtn.parents('tr').removeClass('on');
...
function ShowPop(val) {
    ...
    $('.popup_mask').show();
    Conpop.show();
    Conpop.css('top', Math.max(0,(($(window).height()-$(Conpop).outerHeight())/2) + $(window).scrollTop())+'px');
    Conpop.css('left', Math.max(0,(($(window).width()-$(Conpop).outerWidth())/2) + $(window).scrollLeft())+'px');
return this;
    // switch (val) {
    //     case '0':
...
function login(val) {   
  ...
  const $id = /^(?=.*[0-9]+)[a-zA-Z][a-zA-Z0-9]{5,20}$/g //아이디
  const $pw = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{10,20}$/ //최소한개숫자,특문포함
    ...
    switch (val) {
        ...
        var result = txtFieldCheck() == true ? true : false;
            if(!result){
                location.href="findResultId.html"
            }
        break;
        case 'findPw': //비밀번호찾기
        var result = txtFieldCheck() == true ? true : false;
...
function agree(val){
    switch (val) {
        case 'all'://전체동의
            const allcked = $('#chkAll').is(":checked");
            const checked = $('#chkAll').parents("label").siblings().children('input');
            const check = checked.is(":checked");
            if(allcked){
        ...
        break;
        case 'next'://다음
            const checked1 = $('.check_One input:first').is(':checked');
            const checked2 = $('.check_One input:last').is(':checked');
            if(!checked1 || !checked2){
                alert('동의하지 않은 필수약관이 있습니다.');
            }
            else {
                location.href = 'joinCo.html';
            }   
        break;
        default:
        break;
