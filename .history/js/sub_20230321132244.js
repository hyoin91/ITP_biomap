...
function join() {
    ...
    const name = document.getElementById("name");//메일
    const part = document.getElementById("part");//메일
    ...
    // }
    if(Email.value.search(e_RegExp)) {
        alert('이메일 형식이 옳바르지 않습니다.');
        Email.focus();
        return false;
    }
    if( name.value == '' || part.value == '' || Email.value == '') {
        alert('필수정보를 입력해주세요.');
        name.focus();
        return false;
    }
