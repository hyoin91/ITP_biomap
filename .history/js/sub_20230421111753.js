...
function login(val) {   
  ...
  const EmailCoc = document.getElementById("mailCoc");//메일
  ...
  let $pw = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{10,20}$/ //최소한개숫자,특문포함
    ...
    switch (val) {
        ...
        break;
        case 'doCodePer'://회원가입 이메일본인인증
             if(EmailPer.value.search(e_RegExp) || EmailPer.value == '')  {
...
function doCodeCheck(val) { 
    if(Email.value.search(RegExp) && !Email.value =='')  {
     alert('이메일 형식이 올바르지 않습니다.');
     Email.focus();
     return ;
 }
